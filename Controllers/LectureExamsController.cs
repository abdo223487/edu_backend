...
public class LectureExamsController : ControllerBase
{
    ...
    private async Task<List<Student>> GetEligibleStudentsForLectureAsync(int lectureId)
    {
        ...
        // External-book lectures have no GroupIds concept of their own (see
        // LecturesController.EditLecture's GroupIds guard), so any student
        // who can reach the book gets the assignment/exam -- direct
        // redeemed-code subscription OR access via the book's linked Unit.
        if (lecture.ExternalBookId.HasValue)
        {
